...
/** Utility: fail if *both* phone & email are empty */
function missingContact(phone: string, email: string) {
...
export async function POST(req: Request) {
  ...
  if (!name || !message || missingContact(phone, email)) {
    return new Response(
      JSON.stringify({
        error: "Name, message and at least one contact field are required.",
      }),
      { status: 400 }
    );
  }

  /* ---------- 1 · Notify Royal Aqua admin ---------- */
  ...
  /* ---------- 2 · Optional confirmation to the user (if they gave email) ---------- */
  ...
  if (email) {
    const userHtml = /* html */ `
      ...
      <div style="max-width:560px;margin:auto;padding:32px;border:1px solid #eee;border-radius:8px">
        ...
        <p style="font-size:15px;line-height:1.6">
          Hi ${name.split(" ")[0]},<br/>
          We’ve received your enquiry and will call or email you soon.<br/><br/>
    ...
    ({ error: userError } = await resend.emails.send({
      from: `Royal Aqua <${process.env.RESEND_FROM_EMAIL}>`,
      to: email,
